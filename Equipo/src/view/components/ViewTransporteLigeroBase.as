package view.components
{
	...
	public class ViewTransporteLigeroBase extends VBox
	{
		/***********	VARIABLE'S	*****************/
		[Bindable]public var titlePanel:String = "Transporte Ligero";
		...
		[Bindable]public var acVerificacion:ArrayCollection = new ArrayCollection([{name:'00'},{name:'0'}, {name:'1'},{name:'2'}]);
		...
		public var viewEquipo:ViewTransporteLigero = ViewTransporteLigero(this);
		...
		public function preInit():void{
			...
			subCategoriaTemp.idCategoria = "3";
		...
		protected function selectedItem(_event:Event):void{
			...
			for each(var _verif:Object in acVerificacion){
				if(_verif.name == itemSelected.verificacion){
					viewEquipo.cmbVerificacion.selectedItem = _verif;
					break;
				}
			}
			...
			viewEquipo.dtf_verificacion.selectedDate = DateField.stringToDate(itemSelected.fechaverificacion, "YYYY/MM/DD");
		...
		public function saveUpdate():void
		{
			...
			//Verificación
			itemSelected.verificacion = viewEquipo.cmbVerificacion.selectedItem.name;
			//Fecha Verificación
			var tempDateVerificacion:Date = viewEquipo.dtf_verificacion.selectedDate;
			if(tempDateVerificacion == null){
				//view.dtf_adquisicion.text = "Fecha:__ ___ ___";
				viewEquipo.dtf_verificacion.errorString = "Debes ingresar la fecha de adquisición."
			}else{
				itemSelected.fechaverificacion = tempDateVerificacion.getFullYear().toString() +	'/' + (tempDateVerificacion.getMonth()+1).toString() + '/' + tempDateVerificacion.getDate();
				viewEquipo.dtf_verificacion.errorString = "";
			}
			//Modelo
			...
			trace("itemSelected.modelomotor: " + itemSelected.modelomotor);
			trace("itemSelected.numeromotor: " + itemSelected.numeromotor);
			trace("End");
		}
		...
		public function cleanFormulario():void
		{
			...
			//Verificación
			viewEquipo.cmbVerificacion.selectedItem = null;
			//Fecha Verificación
			viewEquipo.dtf_verificacion.selectedDate = new Date();
